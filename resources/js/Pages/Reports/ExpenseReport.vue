...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: 'Reports', href: route('reports.index') }, { label: 'Expense Report' }]">
        <template #header>
            <PageHeader title="Expense Report">
                <template #actions>
                    <ExportCsvButton route-name="reports.expenses" :params="{ from, to }" />
                </template>
            </PageHeader>
        </template>
        ...
        <div class="mb-4 flex flex-wrap items-end gap-3 rounded-lg bg-white p-4 shadow-sm">
            ...
            <p class="text-xs text-gray-400">{{ count }} expense(s) in this period.</p>
        </div>
