...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: 'Reports', href: route('reports.index') }, { label: 'Purchase Report' }]">
        <template #header>
            <PageHeader title="Purchase Report">
                <template #actions>
                    <ExportCsvButton route-name="reports.purchases" :params="{ from, to }" />
                </template>
            </PageHeader>
        </template>
        ...
        <div class="mb-4 flex flex-wrap items-end gap-3 rounded-lg bg-white p-4 shadow-sm">
            ...
            <p class="text-xs text-gray-400">{{ count }} posted bill(s) in this period.</p>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Bills</th>
                            <th class="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Total Purchases</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200">
                        <tr v-for="row in rows" :key="row.vendor">
                            <td class="whitespace-nowrap px-4 py-3 text-sm text-gray-700">{{ row.vendor }}</td>
                        ...
                        <tr v-if="rows.length === 0">
                            <td colspan="3" class="px-4 py-6 text-center text-sm text-gray-500">No purchases in this period.</td>
                        </tr>
                    </tbody>
