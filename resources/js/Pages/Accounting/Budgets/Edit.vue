...
const form = useForm({
    ...
    amounts: Object.fromEntries(
        [...props.incomeAccounts, ...props.expenseAccounts].map((a) => [a.id, props.budget.amounts[a.id] ?? '0'])
    ),
});
...
function submit() {
    ...
    })).put(route('accounting.budgets.update', props.budget.id));
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: 'Accounting' }, { label: 'Budgets', href: route('accounting.budgets.index') }, { label: budget.name }]">
        <template #header>
