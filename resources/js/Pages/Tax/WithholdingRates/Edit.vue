...
const form = useForm({
    ...
    liability_account_id: props.rate.liability_account_id,
    is_active: props.rate.is_active,
});

function submit() {
    form.put(route('tax.withholding-rates.update', props.rate.id));
}
</script>

<template>
    <Head title="Edit Withholding Tax Rate" />
    ...
    >
        <template #header>
            <PageHeader :title="`Edit Withholding Rate — ${rate.name}`" />
        </template>
