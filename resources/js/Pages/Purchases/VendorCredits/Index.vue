...
watch([search, status], () => {
    router.get(
        route('purchases.vendor-credits.index'),
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: 'Purchases' }, { label: 'Vendor Credits' }]">
        <template #header>
            <PageHeader title="Vendor Credits">
                <template #actions>
                    <Link :href="route('purchases.vendor-credits.create')">
                        <PrimaryButton>New Vendor Credit</PrimaryButton>
                    </Link>
                </template>
            </PageHeader>
        </template>
        ...
        <div v-if="vendorCredits.links?.length > 3" class="mt-4 flex flex-wrap gap-1">
            <Link
                v-for="(link, index) in vendorCredits.links"
                :key="index"
                :href="link.url ?? '#'"
                v-html="link.label"
