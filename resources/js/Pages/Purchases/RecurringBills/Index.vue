...
function generate(template) {
    router.post(route('purchases.recurring-bills.generate', template.id));
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ label: 'Purchases' }, { label: 'Recurring Bills' }]">
        <template #header>
            <PageHeader title="Recurring Bills">
                <template #actions>
                    <Link :href="route('purchases.recurring-bills.create')">
        ...
        <Card>
            ...
            <EmptyState
                ...
                description="Save a template once, then click Generate Now whenever this bill recurs."
            />
        </Card>
    </AppLayout>
</template>
