...
<template>
    <Head title="New Attendance Device" />
    ...
    >
        <template #header>
            <PageHeader title="New Attendance Device" />
        </template>
        ...
        <form class="max-w-xl rounded-lg bg-white p-6 shadow-sm" @submit.prevent="submit">
            <div class="space-y-6">
                <div>
                    ...
                    <TextInput id="name" v-model="form.name" type="text" placeholder="e.g. Main Office Entrance" class="mt-1 block w-full" required />
                    <InputError :message="form.errors.name" class="mt-2" />
                </div>

                <p class="text-xs text-gray-500">
                    Fill in either an IP address (the app connects out to the device — "pull" mode) or a serial number
                    (the device dials home to the app — "push"/ADMS mode), not both.
                </p>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <InputLabel for="ip" value="IP Address" />
                        <TextInput id="ip" v-model="form.ip" type="text" placeholder="192.168.1.201" class="mt-1 block w-full" />
                ...
                <div>
                    ...
                    <TextInput id="model" v-model="form.model" type="text" placeholder="e.g. ZKTeco uFace 800" class="mt-1 block w-full" />
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <PrimaryButton :loading="form.processing">Save Device</PrimaryButton>
            </div>
        </form>
    </AppLayout>
</template>
