...
Not implemented yet: wiring Product selection into Invoice/Bill line items (auto-filling account/price and triggering a stock movement + COGS posting when an inventory-tracked product is actually sold or purchased) — Inventory exists as a correct, standalone module first; this integration is the natural next increment, deliberately not bundled in here, same sequencing as Payments following Invoices rather than shipping together. Also not implemented: Tax on products (Phase 9 — now implemented for Invoices/Bills directly), Reconciliation (Phase 7, still open).

## Phase 9 — Tax / VAT: Tax Rates, Tax Calculation & Tax Reporting Implemented ("Tax Inclusive" pricing not yet)

* Migrations: `tax_rates` (name, rate as percentage, `tax_account_id` must be a liability account); `invoice_items`/`bill_items` gained optional `tax_rate_id` + `tax_amount`; `invoices`/`bills` gained `tax_total`. All additions are nullable/zero-defaulted, so every existing Invoice/Bill test kept passing unmodified — this was verified, not assumed.
* Model: `App\Models\Tax\TaxRate`. **Exclusive tax only** (`TaxRate::calculate()` computes tax on top of the net line amount) — "Tax Inclusive" pricing (backing tax out of a gross price) is a documented scope limit, not attempted, since getting that split wrong silently corrupts revenue recognition and no requirement for it has been confirmed (Section 33: "do not implement jurisdiction-specific rules without verified requirements").
* **Single shared tax account, not separate input/output accounts** — a deliberate, jurisdiction-neutral simplification: `PostInvoice` credits the tax account (output tax collected on a sale); `PostBill` debits the same account (input tax paid on a purchase, offsetting it). The account's own balance is therefore always the current net tax payable — verified directly in `BillTaxTest::net_tax_liability_nets_output_against_input_on_the_same_account`, no separate reporting logic needed to prove it nets correctly. This is a reasonable foundation for jurisdiction-specific rules (e.g. Bangladesh VAT/Mushak) later, without having guessed at any of them now.
* `SaveInvoiceDraft`/`SaveBillDraft` compute `tax_amount` per line and `tax_total` for the document; `PostInvoice`/`PostBill` recompute both fresh from each item's tax rate at posting time (same self-healing discipline as `subtotal`/`discount_total`/`total`) and add one journal line per distinct tax account used (grouped, in case multiple rates ever shared an account).
* Tax Report (`tax.report`): collected (from posted Invoice items) vs. paid (from posted Bill items) vs. net, grouped by tax rate, for an optional date range — built entirely from already-posted item `tax_amount` fields (Section 68: reports must use authoritative accounting data, never a duplicated manual total), the same reuse principle as Section 31's Bank Accounts overview reusing the General Ledger.
* UI: Tax Rates (full CRUD, delete blocked if used on any invoice/bill), a "Tax" column added to both the Invoice and Bill line-item forms (client-side total preview now includes tax, still feedback-only) and both Show pages (per-line tax amount, document tax total).
* Tests: 13 new tests (4 TaxRate, 3 InvoiceTax, 2 BillTax, 2 TaxReport, plus 2 backward-compatibility checks folded into the Invoice/Bill tax tests confirming untaxed documents behave exactly as before). Full suite: 167 tests passing.
* Demo data: `TaxRateSeeder` (VAT 15%, posting to the seeded Tax Payable account) and a second demo invoice (`TaxDemoInvoiceSeeder`, kept separate from the existing untaxed demo invoice so both paths stay visible) — a 200 service line taxed at 15% posts correctly as subtotal 200 / tax 30 / total 230, verified against a fresh reseed.
...
* **Balance Sheet** (as-of date): Assets/Liabilities/Equity account balances. Since there is still no period-close/retained-earnings-rollover mechanism (Section 25 Period Lock, added later, blocks *posting* into a closed period but does not zero P&L into Retained Earnings via closing entries), accumulated net income to date is folded into Equity as its own "Current Period Earnings" line — the standard way to keep Assets = Liabilities + Equity true without a closing-entry feature. **Verified against the full demo dataset spanning every phase built so far:** `Assets − (Liabilities + Equity + Income − Expenses) = 0` exactly, confirming the whole accounting engine nets correctly end to end, not just in isolated tests.
...
## Phase 11 — Security & Audit: Implemented

`spatie/laravel-permission` (^8.3) and `spatie/laravel-activitylog` (^5.1) added, each verified compatible with Laravel 13/PHP 8.4 via `composer require --dry-run` before installing (Section 38 "add only if justified").

* **Permission model**: two-tier `{module}.view` / `{module}.manage` per module (accounts, journal, customers, vendors, invoices, bills, payments, expenses, banking, inventory, tax, users, roles) plus three standalone view-only permissions (`dashboard.view`, `reports.view`, `audit.view`) — 29 permissions total. A 4-tier CRUD-level model (view/create/update/delete) was deliberately rejected as disproportionate complexity for the app's current size.
* **Roles**: Administrator (all permissions), Accountant, Sales, Purchase, Inventory (each a realistic module subset), and Viewer (every `*.view` permission, read-only across the whole app). A "Manager" role was considered and dropped — under this two-tier model it would be permission-identical to Viewer, so keeping both would just be two names for one set (Section 36: "do not create roles blindly").
* **Route gating**: every route file (`accounting`, `contacts`, `sales`, `purchases`, `expenses`, `banking`, `inventory`, `tax`, `reports`, `security`, plus the dashboard route in `web.php`) wrapped in `permission:{module}.view` / `permission:{module}.manage` middleware.
  * **Bug found and fixed**: splitting `Route::resource()` into separate view/manage middleware groups broke every module that has both a `create` route and a wildcard `show` route (e.g. `journals/{journal}`). Laravel matches GET routes in registration order, not specificity, so a `show` route registered before `create` swallowed `/module/create` as `{id}="create"`. Fixed by interleaving registration as index → create/store (manage) → show (view), matching Laravel's own resource-route order. Verified with an HTTP-level regression test (`RouteOrderingTest`, 7 cases) asserting `/module/create` actually renders the Create Inertia component, not a failed model-binding lookup.
* **Sidebar**: now permission-aware — `HandleInertiaRequests` shares the authenticated user's full permission list (`auth.permissions`), and `Sidebar.vue` filters both nav groups and individual links against it, so a role only ever sees links it can actually open.
* **Security UI**: `Security/Roles/{Index,Create,Edit}.vue` (permission checkboxes grouped by module) and `Security/Users/{Index,Edit}.vue` (role assignment). `RoleController::destroy()` blocks deleting the Administrator role or any role currently assigned to a user. `UserController::update()` blocks a user from removing their own Administrator role (self-lockout guard).
* **Audit logging**: `LogsActivity` + `getActivitylogOptions()` (log fillable, only-dirty, skip empty changes) added to every model that represents a financial transaction or master-data record: `Account`, `Journal`, `Invoice`, `Bill`, `Payment`, `VendorPayment`, `Expense`, `Transfer`, `Customer`, `Vendor`, `Product`, `TaxRate`, and `User` (name/email only — password/remember_token excluded). A new `Security/AuditLog/Index` page (gated by `audit.view`, Administrator-only by default) lists every recorded activity with event/subject/causer/attribute-change filtering.
* **Demo data**: a second seeded user, `accountant@example.com` / `password`, assigned the Accountant role — demonstrates differentiated access alongside `test@example.com` (Administrator).
* **Test infrastructure change**: since every route now requires a permission, the base `Tests\TestCase` auto-seeds `PermissionSeeder`/`RoleSeeder` for any test using `RefreshDatabase`, and `UserFactory` auto-assigns the Administrator role to every factory-made user (existing tests were written pre-permissions and assume an unrestricted actor). Tests exercising the permission matrix itself override this with `$user->syncRoles([...])`.
* Tests: 25 new tests in `tests/Feature/Security/` (permission gating across Viewer/Administrator/Sales/no-role users, the route-ordering regression, Role CRUD including the Administrator/assigned-role delete guards, User role-assignment including the self-lockout guard, and audit-log recording + page gating). Full suite: 209 tests passing.
...
**Scoped and implemented, per explicit repo-owner decisions on each open question above:**

* **Recurring Invoices** — manual "Generate Now" trigger only (no scheduler-driven automation, per the repo owner's explicit choice to avoid depending on a cron/task-scheduler that isn't guaranteed to run on this XAMPP/Windows dev setup). `recurring_invoices`/`recurring_invoice_items` store a reusable template (customer, receivable account, line items) with no dates and no totals — totals are never computed or stored on the template itself. `GenerateInvoiceFromRecurring` builds a normal draft-invoice payload from the template and hands it to the existing `SaveInvoiceDraft`, so numbering and totals are computed exactly the same way as a manually created invoice, and the generated invoice still goes through the normal draft → review → post lifecycle (Section 20) — nothing is auto-posted. Scoped to Invoices only (Expenses recurring was explicitly declined). UI: `Sales > Recurring Invoices`, reusing `InvoiceForm.vue` via a new `showDates` prop rather than duplicating the line-item table markup. Tests: 7, covering template CRUD, generation producing correct totals from the template's items, generating twice creating two independent invoices, and that deleting a template doesn't touch invoices already generated from it.
* **In-App Notifications** — computed live on every request to `GET /notifications`, not persisted or scheduler-populated (same reasoning as Recurring's manual trigger: no background job can be relied on to run here). Reports three categories — overdue unpaid Invoices, overdue unpaid Bills, low-stock Products — each included only if the viewer holds the matching module's `.view` permission, so the bell never surfaces data the sidebar itself would hide. Rendered as a bell icon + badge count in `Navbar.vue` (`NotificationBell.vue`), fetched via axios on mount. Deliberately NOT built: email delivery (repo owner chose in-app only, avoiding an SMTP configuration dependency), a persisted/dismissible notification log (would need the scheduler this whole feature was designed to avoid depending on). Tests: 6, covering each category's inclusion/exclusion logic and permission filtering.
* **PDF Export** — `barryvdh/laravel-dompdf` (^3.1), chosen over CSV (the other option offered) since the repo owner's pick was a printable/shareable document, not a raw data export. Two plain Blade views (`resources/views/pdfs/invoice.blade.php`, `bill.blade.php` — PDFs are server-rendered HTML, not Inertia/Vue pages) with a "Download PDF" button on both Show pages. Scoped to Invoices and Bills only, per Section 59 "do not implement every format prematurely" — CSV/Excel and other document types (Customers, Reports) remain unimplemented until requested. Tests: 2, asserting a real PDF (verified by byte content, not just the response header) is returned for both a draft and the same download path works after posting.
* **Scheduled Reports — explicitly skipped**, per the repo owner's own reasoning: it is the most scheduler-dependent of the four Phase 12 items, and reliably firing on a fixed time requires a real OS-level cron/Task Scheduler entry calling `php artisan schedule:run` every minute, which cannot be set up from within this session and was the exact dependency the other three items were designed to avoid. The Reports module itself remains fully usable on-demand; only the "send on a schedule" wrapper around it was declined.

Full regression after all four decisions: 228/228 tests passing, `./vendor/bin/pint --test` clean, `npm run build` clean, `php artisan migrate:fresh --seed --force` clean, and live curl smoke tests confirming a real ~3KB PDF downloads and the notifications endpoint responds correctly against seeded demo data.

## Post-Phase-12 — Frontend Polish, Dashboard, and a Real Date-Boundary Bug Fix

**Frontend design system:** added `Toast`/`Card`/`Badge`/`EmptyState` shared components plus a `loading` prop on `PrimaryButton`/`DangerButton`, rolled out across all 20 Index pages and all 35 Create/Edit/Auth/Profile forms. Converted 12 controller call sites from misusing `withErrors()` as a generic message channel to a proper `flash('error', ...)` that now surfaces as a toast (`app/Http/Middleware/HandleInertiaRequests.php` shares `flash.error` alongside the existing `flash.success`). Unified `PrimaryButton` from Breeze's default near-black to indigo-600, matching every other "primary" accent already in use. Replaced the default Laravel Breeze welcome splash at `/` with an auth-aware redirect (`/dashboard` or `/login`) and removed `Welcome.vue`.

**Dashboard (`DashboardController`):** a QuickBooks-style "money at a glance" panel — six KPI stat cards (Bank Balance, This Month's Income/Expense/Net Profit, AR/AP Outstanding) computed via the same bulk-query patterns as the Reports module, plus a `ModuleFlowDiagram` component showing how Sales (Customers → Invoices → Payments) and Purchases (Vendors → Bills → Payments) both settle through Bank Accounts and post to Journal → Ledger → Reports. Every node is a permission-aware link to that module (reuses the Sidebar's `can()` pattern), and lanes/nodes the current user can't access are hidden rather than shown disabled. Icon path data was extracted from `Sidebar.vue` into a shared `resources/js/icons.js` to avoid duplicating it a second time.

**Bug found and fixed while building the Dashboard's "as of today" KPIs:** `journal_entries.date` is declared as a SQL `DATE` column, but Eloquent's `'date'` cast only affects *read* formatting — on write it still serializes through `getDateFormat()`, which defaults to a full datetime (`Y-m-d H:i:s`). MySQL/MariaDB silently truncates that to a date on insert (so this was invisible against the app's real database), but SQLite (used by the entire test suite) stores it verbatim as text. Every date-range query that did `where('date', '<=', $to)` was therefore doing a **string** comparison: `'2026-09-22 00:00:00' <= '2026-09-22'` is false, since the left side is lexicographically longer with an identical prefix. The practical effect: any report queried "as of today" or "up to today" silently excluded every transaction dated today, under SQLite specifically. This had been latent through Phases 2–10 because no existing test happened to query an exact same-day boundary — the Dashboard's MTD KPIs were the first feature to do so, and its own test caught it immediately (`bankBalance` came back `0.0000` instead of the expected `300.0000`).

Fixed by switching every such comparison to `whereDate('date', ...)` (which extracts just the date part regardless of what's stored) in five places: `Account::netMovement()` (the shared helper behind `balanceAsOf()`, used by Cash Flow, Balance Sheet, and now the Dashboard), `GeneralLedgerController`, `TrialBalanceController`, `ReportController::accountAmountsByType()` (P&L, Balance Sheet), and `DashboardController`. Full regression re-run after the fix: 213/213 tests passing, including three new tests in `tests/Feature/DashboardTest.php` that specifically assert the same-day boundary is inclusive.
